 ...
 * 其中 N = rooms.length。 钥匙 rooms[i][j] = v 可以打开编号为 v 的房间。最初，除 0 号房间外的其余所有房间都被锁住。
...
class Solution {
public:
    bool m_visited[1000] = {false};

    void dfs(int roomID, vector<vector<int>>& rooms) {
        if (!m_visited[roomID]) {
            m_visited[roomID] = true;
            for (int i = 0; i < rooms[roomID].size(); ++i) {
                if (!m_visited[rooms[roomID][i]]) {
                    dfs(rooms[roomID][i], rooms);
                }
            }
        }
    }
