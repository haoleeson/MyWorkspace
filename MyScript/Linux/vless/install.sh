...
acmeInstallSSL() {
    if [[ "${dnsSSLStatus}" == "true" ]]; then
        ...
        echoContent green " ---> 生成证书中(standalone)"
        sudo "$HOME/.acme.sh/acme.sh" --issue -d "${tlsDomain}" --standalone --httpport 8443 -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/v2ray-agent/tls/acme.log >/dev/null
    fi
}
# 自定义端口
customPortFunction() {
